...
IMMUTABILITY_SQL = """
CREATE OR REPLACE FUNCTION ivadoo_reject_audit_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE EXCEPTION 'Ivadoo audit events are immutable'
        USING ERRCODE = '55000';
END;
$$;

CREATE TRIGGER ivadoo_audit_event_immutable
BEFORE UPDATE OR DELETE ON audit_auditevent
FOR EACH ROW
EXECUTE FUNCTION ivadoo_reject_audit_mutation();
"""

REVERSE_IMMUTABILITY_SQL = """
DROP TRIGGER IF EXISTS ivadoo_audit_event_immutable ON audit_auditevent;
DROP FUNCTION IF EXISTS ivadoo_reject_audit_mutation();
"""
