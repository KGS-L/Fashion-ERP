...
def _fernet() -> Fernet:
    try:
        return Fernet(settings.IVADOO_2FA_ENCRYPTION_KEY.encode("ascii"))
    except (ValueError, TypeError) as exc:
        raise RuntimeError(
            "IVADOO_2FA_ENCRYPTION_KEY must be a valid Fernet key."
        ) from exc
...
def begin_totp_setup(user: User) -> tuple[TotpCredential, str, str]:
    ...
    uri = pyotp.TOTP(secret).provisioning_uri(
        name=user.username,
        issuer_name=settings.IVADOO_TOTP_ISSUER,
    )
...
def create_api_session(
    *,
    ...
    absolute_lifetime = timedelta(
        seconds=settings.IVADOO_SESSION_ABSOLUTE_TTL_SECONDS
    )
    idle_lifetime = timedelta(
        seconds=settings.IVADOO_SESSION_IDLE_TTL_SECONDS
    )
