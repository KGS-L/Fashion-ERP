...
def transition_quote(request,quotation_id,target):
    q=scoped(Quotation,request.user,"fashion.sale.manage").get(id=quotation_id); allowed={"send":("draft","sent"),"accept":("sent","accepted"),"reject":("sent","rejected"),"expire":("sent","expired")}
    source,dest=allowed[target]
    if q.status!=source: raise ValidationError({"status":f"Cannot {target} quotation from {q.status}."})
    q.status=dest;q.save(update_fields=["status","updated_at"]);record_audit_event(organization=request.user.organization,actor=request.user,action=f"fashion.quotation.{target}",object_instance=q,request=request);return Response(QuotationSerializer(q).data)
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def quotation_action(request,quotation_id,action):
    if action not in {"send","accept","reject","expire"}: return Response(status=status.HTTP_404_NOT_FOUND)
    return transition_quote(request,quotation_id,action)
