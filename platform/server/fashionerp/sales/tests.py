...
class FashionSalesFlowTests(APITestCase):
 ...
 def test_full_quote_order_confirmation_freezes_measurements(self):
  q=self.client.post("/api/v1/sales/quotations/",{"company":str(self.company.id),"customer":str(self.customer.id),"currency":str(self.currency.id),"number":"Q-001","lines":[{"description":"Custom garment","quantity":"1","unit_price":"50000","discount_rate":"0"}]},format="json");self.assertEqual(q.status_code,status.HTTP_201_CREATED)
 ...
 def test_status_cannot_be_patched_through_collection_payload(self):
  q=self.client.post("/api/v1/sales/quotations/",{"company":str(self.company.id),"customer":str(self.customer.id),"currency":str(self.currency.id),"number":"Q-002","status":"accepted","lines":[]},format="json");self.assertEqual(q.data["status"],"draft")
