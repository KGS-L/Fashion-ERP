...
FashionERP records sensitive Foundation operations in an append-only PostgreSQL audit journal.
...
2. PostgreSQL rejects `UPDATE` and `DELETE` through the `fashionerp_audit_event_immutable` trigger.
...
The FashionERP specification requires configurable retention for journals, but does not yet define retention periods, legal holds or privileged purge rules.
